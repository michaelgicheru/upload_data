...
offline_update <- function(new_data, system, data_type, schema_table) {
	...
	current_path <- here::here(
		input_dir,
		glue("Offline {system} {data_type}_{current_suffix}.csv")
	)
	initial_data <- nanoparquet::read_parquet(file = prior_path)

	# Import New Data
	new_data <- import_data(system = system, data_type = data_type)

	out <- bind_rows(initial_data, new_data)

	write_csv(x = out, file = current_path)
